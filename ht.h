...
public:
    ...
    // To be completed
    HASH_INDEX_T next()  //add override?
    {
         if (this->numProbes_ >= this->m_) {
        return this->npos;
    }
...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober),totalProbes_(0), mIndex_(0), resizeAlpha_(resizeAlpha), numItems_(0), numDeleted_(0)
{
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if (loc == npos) {
        throw std::logic_error("No free location found for insertion.");
    }
